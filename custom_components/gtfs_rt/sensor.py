...
ATTR_SECOND_MIN = "Second train" #Added
ATTR_THIRD_MIN = "Third train" #Added
ATTR_FOURTH_MIN = "Fourth train" #Added
ATTR_ICON = "Icon"
...
class PublicTransportSensor(Entity):
    ...
    def extra_state_attributes(self):
        ...
        if len(next_services):
            attrs[ATTR_NEXT_UP] = (
                next_services[1].arrival_time.strftime(TIME_STR_FORMAT)
                if len(next_services) > 1
                else "-"
            )
# Add additional trains in minutes
        if len(next_services):
            attrs[ATTR_SECOND_MIN] = (
                due_in_minutes(next_services[1].arrival_time)
                if len(next_services) > 1
                else "-"
            )
        if len(next_services):
            attrs[ATTR_THIRD_MIN] = (
                due_in_minutes(next_services[2].arrival_time)
                if len(next_services) > 2
                else "-"
            )
        if len(next_services):
            attrs[ATTR_FOURTH_MIN] = (
                due_in_minutes(next_services[3].arrival_time)
                if len(next_services) > 3
                else "-"
            )
        return attrs
...
class PublicTransportData(object):
    ...
    def __init__(
        ...
        if api_key is not None:
            #self._headers = {"Authorization": api_key} Changed to api_key for WMATA
